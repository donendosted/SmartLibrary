  ...
  const filter = req.query.q
    ? {
        $or: [
          ...
          { author: { $regex: String(req.query.q), $options: "i" } },
        ],
      }
    : {};
  ...
  const data = await Promise.all(
    users.map(async (user) => {
      const transactions = await db()
        ...
        .find({ user_id: user._id.toString(), status: "active" })
        .toArray();
      ...
      return {
        ...serialize(user),
        books_borrowed: transactions.length,
        outstanding_fines: fines.reduce((sum, fine) => sum + fine.amount, 0),
      };
    }),
  );
...
router.get("/users/:id", async (req, res) => {
  if (!isId(req.params.id))
    throw apiError(404, "Student not found", "STUDENT_NOT_FOUND");
  const database = db();
  ...
  res.json({ ...serialize(user), history });
});
...
router.get("/reports/overdue", async (_req, res) => {
  const database = db();
  const transactions = await database
    .collection("transactions")
...
router.post("/book", async (req, res) => {
  const { isbn, title, author, category, description, quantity = 1 } = req.body;
  if (!title || !author)
    throw apiError(400, "title and author are required", "VALIDATION_ERROR");
  const book = {
    isbn: isbn || undefined,
    title,
    author,
  ...
  const copies = Array.from(
    { length: Math.max(1, Number(quantity)) },
    (_, index) => ({
      book_id: book._id.toString(),
      barcode: `${isbn || "BOOK"}-${book._id}-${index + 1}`,
      status: "available",
      created_at: new Date(),
    }),
  );
...
router.post("/book/import", upload.single("file"), async (req, res) => {
  if (!req.file) throw apiError(400, "CSV file required", "VALIDATION_ERROR");
  ...
  for (const record of records) {
    ...
    const book = result;
    const copies = Array.from(
      { length: Math.max(1, Number(record.Quantity || 1)) },
      (_, index) => ({
        book_id: book._id.toString(),
        barcode: `${record.ISBN}-${Date.now()}-${index}`,
        status: "available",
        created_at: new Date(),
      }),
    );
