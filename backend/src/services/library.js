...
async function notify(user_id, type, message) {
  await db()
    .collection("notifications")
    .insertOne({ user_id, type, message, created_at: now() });
}
...
export async function checkout({ barcode, student_id }) {
  ...
  const student = await database
    .collection("users")
    .findOne({ student_id, role: "student" });
  ...
  if (!copy) {
    ...
    throw apiError(
      ...
      exists ? "COPY_UNAVAILABLE" : "COPY_NOT_FOUND",
    );
  }
  try {
    const due_date = new Date();
    due_date.setDate(due_date.getDate() + config.loanDurationDays);
    const transaction = {
    ...
    await notify(
      ...
      "checkout",
      `Checked out ${book?.title || "book"}; due ${due_date.toISOString().slice(0, 10)}.`,
    );
...
export async function returnCopy({ barcode }) {
  ...
  const transaction = await database
    ...
    .findOneAndUpdate(
      { copy_id: copy._id.toString(), status: "active" },
  ...
  const overdue = daysLate(transaction.due_date);
  const fine_amount = overdue * config.dailyFineRate;
  if (overdue)
    await database.collection("fines").insertOne({
      transaction_id: transaction._id.toString(),
      amount: fine_amount,
      reason: `${overdue} overdue day(s)`,
      created_at: now(),
    });
  const hold = await database.collection("holds").findOneAndUpdate(
    { book_id: copy.book_id.toString(), status: "waiting" },
    {
      $set: {
        status: "ready",
  ...
  if (hold) {
    ...
    await notify(
      hold.user_id,
      "hold_ready",
      `${book?.title || "A book"} is ready for collection.`,
    );
  }
